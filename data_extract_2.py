...
for i in range(1):
  ...
  while(not guessed):
    ...
    #hints = game_host("eater", word_guess)              #feedback "GGYWW"

    if "G" in hints:
      found_green = True
    
    conditions = conditions & get_conditions(hints, word_guess, df)    #AND all previous conditions
    ...
    for j in range(len(hints)):
      if green_index[j] == 0:
        ...
        if (filtered['letter_' + str(j + 1)].values == filtered['letter_' + str(j + 1)].values[0]).all():
          green_index[j] = 1
          '''
          print(filtered["letter_" + str(j+1)].values[0])
          '''
